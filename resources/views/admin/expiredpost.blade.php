    ...
    <section class="section">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Expired Post</h5>
...
<script>
    $(document).ready(function() {
        ...
        function fetchExpiredPosts() {
            $.ajax({
                url: "{{ url('admin/fetch-expired-posts') }}",
                type: "GET",
                async: false,
                success: function(res) {
                    $('#expiredAdsList').html(``);
                    $.each(res, function(key, value) {
                        var formattedDate = moment(value.posted, "YYYY-MM-DD HH:mm:ss").format("MMM D, YYYY");
                        $('#expiredAdsList').append(`
                            <tr>
                                <td>${value.title}</td>
                                <td>${value.username}</td>
                                <td>${formattedDate}</td>
                                <td style="text-transform: capitalize;" class="text-danger">${value.status}</td>
                                <td>
                                    <a class="btn btn-sm btn-primary view" data="${value.id}">View</a>
                                    <a class="btn btn-sm btn-success approve" data="${value.id}">Approve</a>
                                    <a class="btn btn-sm btn-danger delete" data="${value.id}">Delete</a>
                                </td>
                            </tr>
                        `);
                    });
